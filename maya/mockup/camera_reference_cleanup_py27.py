"""
Camera-Based Reference Cleanup Tool (Python 2.7 Compatible)

Removes references not visible in camera frustum.
...
import maya.OpenMaya as om  # OpenMaya 1.0 for Python 2.7 compatibility
...
class ReferenceCleanupUI:
    ...
    def find_related_geometry_by_name(self, shader_target):
        ...
        print("    Searching for geometry matching shader target: '{}'".format(shader_target))
        ...
        for ref in self.all_references:
            ...
            if geo_name and shader_target and geo_name == shader_target.lower():
                ...
                print("      Found matching geo: {}".format(ref['namespace']))
    ...
    def find_shaders_for_geometry(self, geo_ref_data):
        ...
        print("  Searching for shaders matching geo: '{}'".format(geo_name))
        ...
        for ref in self.all_references:
            ...
            if ref_info['type'] == 'SHADER':
                ...
                print("    Checking shader: {} (target: '{}')".format(ref['namespace'], shader_target))
    ...
    def _auto_restore_matching_shaders(self):
        ...
        """
        print("\n{}".format('='*60))
        print("Auto-Restoring Shaders for Geometry in Keep List")
        print("{}".format('='*60))
        ...
        print("Found {} geometry reference(s) in keep list".format(len(geo_in_keep)))
        ...
        print("Unique geo names to match: {}".format(geo_names_in_keep))
        ...
        for ref in self.invisible_references[:]:  # Copy list to avoid modification during iteration
            ...
            if ref_info['type'] == 'SHADER':
                ...
                if shader_target and shader_target in geo_names_in_keep:
                    ...
                    print("  -> Will auto-keep shader: {} (matches '{}')".format(ref['namespace'], shader_target))
        ...
        print("\n✓ Auto-restored {} shader(s) to keep list".format(len(shaders_to_move)))
        print("{}\n".format('='*60))
    ...
    def get_ref_from_display_name(self, display_name, ref_list):
        ...
        try:
            ...
            print("Error parsing display name '{}': {}".format(display_name, str(e)))
    ...
    def restore_shaders_for_selected(self):
        ...
        print("\n{}".format('='*60))
        print("Restoring Shaders for Selected Geometry")
        print("{}".format('='*60))
        ...
        for item in selected_items:
            ...
            if ref_data and ref_data['ref_type'].get('is_geo', False):
                ...
                print("\nSelected geo: {}".format(ref_data['namespace']))
        ...
        for geo_ref in selected_geos:
            ...
            print("  Found {} matching shader(s)".format(len(matching_shaders)))
            ...
            for shader_ref in matching_shaders:
                ...
                if self.is_ref_in_list(shader_ref, self.invisible_references):
                    if not self.is_ref_in_list(shader_ref, shaders_to_move):
                        ...
                        print("    -> Will move: {}".format(shader_ref['namespace']))
                else:
                    print("    -> Already in keep list: {}".format(shader_ref['namespace']))
        ...
        cmds.confirmDialog(
            ...
            message='Moved {} shader(s) to Keep list.'.format(len(shaders_to_move)),
            button=['OK']
        )
        
        print("\n✓ Successfully restored {} shader(s)".format(len(shaders_to_move)))
        print("{}\n".format('='*60))
    ...
    def restore_all_shaders(self):
        ...
        """
        print("\n{}".format('='*60))
        print("Restoring ALL Shaders for Geometry in Keep List")
        print("{}".format('='*60))
        ...
        print("\nFound {} geometry reference(s) in keep list".format(len(geo_in_keep)))
        
        # Find all shaders that should be kept
        shaders_to_move = []
        
        for geo_ref in geo_in_keep:
            print("\nChecking geo: {}".format(geo_ref['namespace']))
            matching_shaders = self.find_shaders_for_geometry(geo_ref)
    ...
    def move_to_invisible(self, *args):
        ...
        self.update_reference_lists()
        print("Moved {} reference(s) to Remove list".format(len(refs_to_move)))
    ...
    def move_to_visible(self, *args):
        ...
        self.update_reference_lists()
        print("Moved {} reference(s) to Keep list".format(len(refs_to_move)))
    ...
    def get_selected_camera(self, *args):
        ...
        cmds.text(self.scan_status, edit=True, label="Status: Camera '{}' selected. Ready to scan.".format(camera))
        print("Camera selected: {}".format(camera))
    ...
    def get_camera_frustum_planes(self):
        ...
        try:
            ...
            # OpenMaya 1.0 MMatrix constructor
            mat = om.MMatrix()
            om.MScriptUtil.createMatrixFromList(cam_matrix, mat)

            # Extract vectors from matrix (OpenMaya 1.0 style)
            cam_pos = om.MVector(mat(3, 0), mat(3, 1), mat(3, 2))
            forward = om.MVector(-mat(2, 0), -mat(2, 1), -mat(2, 2)).normal()
            right = om.MVector(mat(0, 0), mat(0, 1), mat(0, 2)).normal()
            up = om.MVector(mat(1, 0), mat(1, 1), mat(1, 2)).normal()
            ...
            print("Error getting camera frustum: {}".format(str(e)))
            return None
    ...
    def is_bbox_in_frustum(self, bbox, frustum):
        ...
        try:
            # OpenMaya 1.0 MVector constructor
            corners = [
                om.MVector(bbox[0], bbox[1], bbox[2]),
                om.MVector(bbox[3], bbox[1], bbox[2]),
                om.MVector(bbox[0], bbox[4], bbox[2]),
                om.MVector(bbox[3], bbox[4], bbox[2]),
                om.MVector(bbox[0], bbox[1], bbox[5]),
                om.MVector(bbox[3], bbox[1], bbox[5]),
                om.MVector(bbox[0], bbox[4], bbox[5]),
                om.MVector(bbox[3], bbox[4], bbox[5])
            ]
    ...
    def get_visible_objects_in_camera(self):
        ...
        try:
            ...
            print("Found {} visible mesh objects in frustum".format(len(visible_objects)))
            return visible_objects
            
        except Exception as e:
            print("Error getting visible objects: {}".format(str(e)))
            return []
    ...
    def scan_references(self, *args):
        ...
        print("\n{}".format('='*60))
        print("Scanning {} references...".format(len(all_refs)))
        print("{}".format('='*60))
        ...
        for ref in all_refs:
            try:
                ...
                if ref_data['ref_type']['type'] == 'CAMERA':
                    ...
                    print("  [CAMERA] {} - always kept".format(ref_namespace))
                    continue
                ...
                if ref_data['ref_type'].get('is_geo', False):
                    ...
                    if is_visible:
                        self.visible_references.append(ref_data)
                        print("  [GEO-KEEP] {} - visible in frustum".format(ref_namespace))
                    else:
                        self.invisible_references.append(ref_data)
                        print("  [GEO-REMOVE] {} - not in frustum".format(ref_namespace))
                else:
                    # Non-geometry (shaders, etc.) - add to invisible for now
                    # They will be auto-restored after geometry scan is complete
                    self.invisible_references.append(ref_data)
                    print("  [{}] {} - pending shader matching".format(ref_data['ref_type']['type'], ref_namespace))

            except Exception as e:
                print("Error processing reference {}: {}".format(ref, str(e)))
                continue
        ...
        status_msg = "Status: {} to keep, {} to remove".format(len(self.visible_references), len(self.invisible_references))
        cmds.text(self.scan_status, edit=True, label=status_msg)

        print("\n{}".format('='*60))
        print("Scan Complete:")
        print("  Total: {}".format(len(self.all_references)))
        print("  Keep: {} (geometry + matching shaders)".format(len(self.visible_references)))
        print("  Remove: {}".format(len(self.invisible_references)))
        print("{}\n".format('='*60))
    ...
    def update_reference_lists(self):
        ...
        cmds.text(self.total_refs_text, edit=True, label="Total: {}".format(len(self.all_references)))
        cmds.text(self.visible_refs_text, edit=True, label="Keep: {}".format(len(self.visible_references)))
        cmds.text(self.invisible_refs_text, edit=True, label="Remove: {}".format(len(self.invisible_references)))
        ...
        for ref_data in self.visible_references:
            if self.should_show_reference(ref_data, 'visible'):
                ref_info = ref_data['ref_type']
                display_name = "[{}] {} - {}".format(ref_info['display_type'], ref_data['namespace'], ref_data['short_name'])
    ...
    def update_remove_count(self, *args):
        ...
        cmds.text(self.remove_count_text, edit=True, label="Ready to remove: {} reference(s)".format(len(selected)))
    ...
    def select_from_viewport(self, list_type):
        ...
        if not items_to_select:
            cmds.warning("No matching references found in {} list for selected objects!".format('Keep' if list_type == 'visible' else 'Remove'))
            return
        ...
        for item in items_to_select:
            try:
                ...
                print("Could not select item: {} - {}".format(item, str(e)))

        print("Selected {} reference(s) in {} list from viewport selection".format(len(items_to_select), 'Keep' if list_type == 'visible' else 'Remove'))
    ...
    def select_reference_in_scene(self, list_type):
        ...
        if nodes_to_select:
            ...
            print("Selected {} node(s) in scene".format(len(nodes_to_select)))
    ...
    def remove_selected_references(self, *args):
        ...
        print("\n{}".format('='*60))
        print("Removing Selected References")
        print("{}".format('='*60))
        ...
        for item in selected_items:
            ...
            if ref_data:
                ...
                print("  Will remove: {} ({})".format(ref_data['namespace'], ref_data['short_name']))
        ...
        result = cmds.confirmDialog(
            ...
            message='Remove {} reference(s) from scene?'.format(len(refs_to_remove)),
            button=['Yes', 'No'],
            defaultButton='Yes',
            cancelButton='No'
        )
        ...
        for ref_data in refs_to_remove:
            ...
            if ref_node in keep_nodes:
                print("⚠ Skipping {} - found in keep list!".format(ref_data['namespace']))
                continue

            try:
                # Check if reference still exists before removing
                if not cmds.objExists(ref_node):
                    print("⚠ Reference node no longer exists: {}".format(ref_node))
                    continue

                # Use referenceNode to remove SPECIFIC reference instance
                cmds.file(referenceNode=ref_node, removeReference=True)
                print("✓ Removed: {}".format(ref_data['namespace']))
                removed_count += 1
            except Exception as e:
                print("✗ Failed to remove {}: {}".format(ref_data['namespace'], str(e)))
                failed_refs.append(ref_data['namespace'])
        
        if failed_refs:
            msg = "Removed {} reference(s).\n\nFailed: {}".format(removed_count, ', '.join(failed_refs))
        else:
            msg = "Successfully removed {} reference(s)!".format(removed_count)
        
        cmds.confirmDialog(title='Complete', message=msg, button=['OK'])
        
        print("\n{}".format('='*60))
        print("Removal Complete: {} removed, {} failed".format(removed_count, len(failed_refs)))
        print("{}\n".format('='*60))
