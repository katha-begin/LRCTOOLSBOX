"""
Reference to Instance Converter Tool for Maya (v2.1 - Python 2.7 Compatible)
...
class HierarchyAnalyzer:
    ...
    def analyze(self, root_group, progress_callback=None):
        ...
        if not cmds.objExists(root_group):
            cmds.warning("Root group '{}' does not exist.".format(root_group))
            return []

        print("[Analyzer] Starting analysis from: {}".format(root_group))

        # Find all potential asset groups (ending with _Grp)
        self._walk_hierarchy(root_group, progress_callback)

        print("[Analyzer] Found {} referenced assets under locators".format(len(self.all_assets)))

        # Group by reference file to find duplicates
        self._group_by_reference()

        print("[Analyzer] Created {} reference groups".format(len(self.duplicate_groups)))
    ...
    def _process_locator(self, locator_path, parent_group):
        ...
        for child_path in children:
            ...
            if ref_info:
                ...
                print("[Analyzer] Found: {} under {}".format(namespace, locator_short))
    ...
    def _group_by_reference(self):
        ...
        for ref_file, assets in file_groups.items():
            ...
            if group.is_duplicate:
                print("[Analyzer] DUPLICATE: {} ({} refs)".format(asset_name, group.count))
...
class InstanceConverter:
    ...
    def convert_group(self, duplicate_group, dry_run=False):
        ...
        if not duplicate_group.is_duplicate:
            self.log("Skipping {} - only one reference".format(duplicate_group.asset_name))
            return False

        self.log("\n{}".format('='*60))
        self.log("Converting: {}".format(duplicate_group.asset_name))
        self.log("Reference file: {}".format(duplicate_group.ref_file))
        self.log("Total instances: {}".format(duplicate_group.count))
        self.log("{}".format('='*60))
        ...
        self.log("\nMaster (will keep): {}".format(master.namespace))
        self.log("  Locator: {}".format(master.locator))
        self.log("  Geo Group: {}".format(master.geo_group))

        if not cmds.objExists(master_geo_path):
            self.log("ERROR: Master geo group does not exist: {}".format(master_geo_path))
            return False

        # Process each slave
        success_count = 0
        for i, slave in enumerate(slaves, 1):
            self.log("\n--- Slave {}/{}: {} ---".format(i, len(slaves), slave.namespace))

            if self._convert_slave_to_instance(slave, master, dry_run):
                success_count += 1
            else:
                self.log("WARNING: Failed to convert slave {}".format(slave.namespace))

        self.log("\n✓ Converted {}/{} slaves to instances".format(success_count, len(slaves)))
        return True
    ...
    def _convert_slave_to_instance(self, slave, master, dry_run=False):
        ...
        self.log("  Locator: {}".format(slave.locator))
        self.log("  Geo Group: {}".format(slave.geo_group))

        # Validate using full paths
        if not cmds.objExists(slave_locator_path):
            self.log("  ERROR: Locator does not exist: {}".format(slave_locator_path))
            return False

        if not cmds.objExists(slave_geo_path):
            self.log("  ERROR: Geo group does not exist: {}".format(slave_geo_path))
            return False
        ...
        try:
            ...
            self.log("  ✓ Removed reference: {}".format(slave.namespace))
    ...
    def convert_multiple_groups(self, groups, progress_callback=None, dry_run=False):
        ...
        self.log("Starting conversion of {} duplicate reference groups...".format(total))

        for i, group in enumerate(duplicate_groups, 1):
            if progress_callback:
                progress_callback(i, total, "Converting: {}".format(group.asset_name))

            self.convert_group(group, dry_run)

        self.log("\n{}".format('='*60))
        self.log("Conversion complete! Processed {} groups.".format(total))
        self.log("{}".format('='*60))
    ...
    def populate(self, duplicate_groups):
        ...
        for group in duplicate_groups:
            ...
            if group.is_duplicate:
                ...
                status = "DUPLICATE ({} refs)".format(group.count)
            ...
            group_item.setText(0, "{} {}".format(icon, group.asset_name))
            ...
            for i, instance in enumerate(group.instances):
                ...
                instance_item.setText(0, "  {}".format(prefix))
...
class ReferenceToInstanceUI(QtWidgets.QDialog):
    ...
    def _on_get_selected(self):
        ...
        if sel:
            ...
            self._log("Root set to: {}".format(sel[0]), "#88cc88")
        else:
    ...
    def _on_analyze(self):
        ...
        if not cmds.objExists(root):
            self._log("Error: '{}' does not exist".format(root), "#ff6666")
            return

        self._log("\nAnalyzing hierarchy under: {}".format(root))
        ...
        try:
            ...
            self._log("[Debug] Found {} referenced assets".format(len(self.all_assets_found())))
            self._log("[Debug] Grouped into {} reference groups".format(len(self.duplicate_groups)))
            ...
            self.stats_label.setText(
                ...
                "{} single refs".format(len(self.duplicate_groups), dup_groups, dup_count, single_count)
            )

            if dup_groups > 0:
                self.stats_label.setStyleSheet("color: #ffaa00; font-weight: bold;")
                self._log("✓ Found {} asset types with duplicate references!".format(dup_groups), "#ffaa00")
            else:
            ...
            import traceback
            self._log("Error during analysis: {}".format(e), "#ff6666")
    ...
    def _on_preview_in_maya(self):
        ...
        if isinstance(data, DuplicateReferenceGroup):
            ...
            if locs:
                ...
                self._log("Selected {} locators".format(len(locs)))
            ...
            if cmds.objExists(loc_path):
                ...
                self._log("Selected: {}".format(data.locator))
    ...
    def _on_convert(self):
        ...
        result = QtWidgets.QMessageBox.question(
            self,
            "Confirm Conversion",
            "This will convert {} references to instances.\n\n"
            "• {} asset types selected\n"
            "• {} references will be removed\n"
            "• {} master references will be kept\n\n"
            "This operation CANNOT be undone!\n\n"
            "Continue?".format(to_convert, len(selected), to_convert, len(selected)),
    ...
    def _execute_conversion(self, groups, dry_run=False):
        ...
        try:
            ...
            import traceback
            self._log("Error: {}".format(e), "#ff6666")
            self._log(traceback.format_exc(), "#ff6666")
        ...
        for i in range(root.childCount()):
            ...
            if item_data and hasattr(item_data, 'ref_file'):
                if normalize_ref_file(item_data.ref_file) == normalize_ref_file(group.ref_file):
                    # Mark as converted
                    item.setText(0, "✓ {} [CONVERTED]".format(group.asset_name))
...
def debug_analyze(root_group):
    ...
    """
    print("\n{}".format('='*60))
    print("DEBUG ANALYSIS: {}".format(root_group))
    print("{}".format('='*60))

    if not cmds.objExists(root_group):
        print("ERROR: Root group does not exist: {}".format(root_group))
        return
    ...
    print("\n[Results]")
    print("Total assets found: {}".format(len(analyzer.all_assets)))
    print("Total reference groups: {}".format(len(groups)))

    for i, asset in enumerate(analyzer.all_assets):
        print("\n  Asset {}:".format(i+1))
        print("    Locator: {}".format(asset.locator))
        print("    Geo: {}".format(asset.geo_group))
        print("    Namespace: {}".format(asset.namespace))
        print("    Ref File: {}".format(asset.ref_file))
        print("    Locator Path: {}".format(asset.locator_path))
        print("    Geo Path: {}".format(asset.geo_group_path))

    print("\n[Reference Groups]")
    for group in groups:
        status = "DUPLICATE" if group.is_duplicate else "Single"
        print("\n  {}: {} refs ({})".format(group.asset_name, group.count, status))
        print("    File: {}".format(group.ref_file))
        for j, inst in enumerate(group.instances):
            role = "Master" if j == 0 else "Slave"
            print("      [{}] {}".format(role, inst.namespace))
